-- Re-create the assistant custom-action library dropped by 0220. One shared
...
-- boolean each in `assignments`). Secret header values are encrypted at rest
-- by the service before insert; the stored value on a secret header is
-- ciphertext.
--
-- Enabled + assigned + the `assistantCustomActions` feature flag together
-- decide whether a definition assembles into an agent's toolset at turn time.
